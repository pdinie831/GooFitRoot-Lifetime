...
EXEC_TARGET fptype device_ExpGausPEESigma (fptype* evt, fptype* p, unsigned int* indices) {
...
//  ret *= (1- ERF(erfarg)); 
  ret *= ERFC(erfarg); 
  if (ret<=0){
    printf("Error: ExpGausPEESigmaPdf<=0!!! ==> x=%f  sigma=%f mean=%f tau=%f ret=%f\n", x, sigma, mean,tau , ret);
  }   
//
    fptype lo = 0.0003;
    fptype hi = 0.007 ;
...
//  printf("\n\nExpGausPEESigmaPdf x=%f  sigma=%f mean=%f tau=%f ret=%f integral =%f\n", x, sigma, mean,tau , ret, integral);
//  printf("ExpGausPEESigmaPdf x=%f  sigma=%f mean=%f tau=%f ret=%f integral1=%f\n", x, sigma, mean,tau , ret, integral1);
//}  
 
/*  if ((0 == THREADIDX) && (0 == BLOCKIDX)){
  printf("ExpGausPEESigmaPdf->integrate  sigma=%f mean=%f tau=%f Integral=%f\n",    sigma, mean,tau , integral);
  printf("ExpGausPEESigmaPdf->integrate  sigma=%f mean=%f tau=%f IntegralLO=%f\n",  sigma, mean,tau , intg_lo);
  printf("ExpGausPEESigmaPdf->integrate  sigma=%f mean=%f tau=%f IntegralHI=%f\n",  sigma, mean,tau , intg_hi);
  printf("ExpGausPEESigmaPdf->integrate  sigma=%f mean=%f tau=%f EXP(expa_hi)=%f\n",  sigma, mean,tau , EXP(expa_hi));
  printf("ExpGausPEESigmaPdf->integrate  sigma=%f mean=%f tau=%f EXP(expa_lo)=%f\n",  sigma, mean,tau , EXP(expa_lo));
 }
 */// printf("ExpGausPEESigmaPdf->host_indices  host_indices0=%d host_indices1=%d host_indices2=%d\n",  host_indices[0], host_indices[1],host_indices[2]);
// printf("ExpGausPEESigmaPdf->     indices  indices0=%d indices1=%d indices2=%d\n",  indices[0], indices[1],indices[2]);
//  if ((0 == THREADIDX) && (0 == BLOCKIDX)){
//   printf("ExpGausPEESigmaPdf x=%f  sigma=%f mean=%f tau=%f ret=%f integral=%f\n", x, sigma, mean,tau , ret, integral);
//    printf("ExpGausPEESigmaPdf ind0 =%d  evt0 =%f \n",indices[0] ,evt[indices[0]] );
//    printf("ExpGausPEESigmaPdf ind1 =%d  evt1 =%f \n",indices[1] ,evt[indices[1]] );
//    printf("ExpGausPEESigmaPdf ind2 =%d  evt2 =%f \n",indices[2] ,evt[indices[2]] );
//    printf("ExpGausPEESigmaPdf ind3 =%d  evt3 =%f \n",indices[3] ,evt[indices[3]] );
//    printf("ExpGausPEESigmaPdf ind4 =%d  evt4 =%f \n",indices[4] ,evt[indices[4]] );
//    printf("ExpGausPEESigmaPdf ind5 =%d  evt5 =%f \n",indices[5] ,evt[indices[5]] );
//    printf("ExpGausPEESigmaPdf ind6 =%d  evt6 =%f \n",indices[6] ,evt[indices[6]] );
//    printf("ExpGausPEESigmaPdf ind7 =%d  evt7 =%f \n",indices[7] ,evt[indices[7]] );
//    printf("ExpGausPEESigmaPdf ind8 =%d  evt8 =%f \n",indices[8] ,evt[indices[8]] );
//    printf("ExpGausPEESigmaPdf ind9 =%d  evt9 =%f \n",indices[9] ,evt[indices[9]] );
//    printf("ExpGausPEESigmaPdf ind10=%d  evt10=%f \n",indices[10],evt[indices[10]] );
//    printf("ExpGausPEESigmaPdf ind11=%d  evt11=%f \n",indices[11],evt[indices[11]] );
//    printf("ExpGausPEESigmaPdf ind12=%d  evt12=%f \n",indices[12],evt[indices[12]] );
//    printf("ExpGausPEESigmaPdf ind13=%d  evt13=%f \n",indices[13],evt[indices[13]] );
//    printf("ExpGausPEESigmaPdf ind14=%d  evt14=%f \n",indices[14],evt[indices[14]] );
// }
//  return 0; 
//
  return ret;
...
MEM_DEVICE device_function_ptr ptr_to_ExpGausPEESigma = device_ExpGausPEESigma; 

 __host__ ExpGausPEESigmaPdf::ExpGausPEESigmaPdf (std::string n, Variable* _x, Variable* _s,  Variable* mean, Variable* tau) 
  : GooPdf(_x, n)
{
...
//  pindices.push_back(registerParameter(sigma));
  ...
  GET_FUNCTION_ADDR(ptr_to_ExpGausPEESigma);
  initialise(pindices); 
}

 __host__ fptype ExpGausPEESigmaPdf::integrate (fptype lo, fptype hi) const {
// printf("integratexxx\n");
