...
EXEC_TARGET fptype device_PolyEffi (fptype* evt, fptype* p, unsigned int* indices) {
  ...
  for (int i = 2; i < numParams; ++i) {
   ...
   if (ret<0) {
     printf("Error: PolyEffiPdf<=0!!! ==> x=%f  p0=%f p1=%f p2=%f p3=%f ret=%f\n", x, p[indices[2]],p[indices[3]],p[indices[4]],p[indices[5]] ,ret);
    ret=0;
   } 
//  return fabs(ret); 
  return ret; 
...
EXEC_TARGET fptype device_OffsetPolyEffi (fptype* evt, fptype* p, unsigned int* indices) {
...
EXEC_TARGET fptype device_MultiPolyEffi (fptype* evt, fptype* p, unsigned int* indices) {
...
MEM_DEVICE device_function_ptr ptr_to_PolyEffi = device_PolyEffi; 
MEM_DEVICE device_function_ptr ptr_to_OffsetPolyEffi = device_OffsetPolyEffi; 
MEM_DEVICE device_function_ptr ptr_to_MultiPolyEffi = device_MultiPolyEffi; 

// Constructor for single-variate PolyEffi, with optional zero point. 
__host__ PolyEffiPdf::PolyEffiPdf (string n, Variable* _x, vector<Variable*> weights, Variable* x0, unsigned int lowestDegree) 
  : GooPdf(_x, n) 
  , center(x0)
{
  ...
  if (x0) {
    ...
    GET_FUNCTION_ADDR(ptr_to_OffsetPolyEffi);
  }
  else {
    GET_FUNCTION_ADDR(ptr_to_PolyEffi);
  }
  initialise(pindices); 
}

// Constructor for multivariate PolyEffi. 
__host__ PolyEffiPdf::PolyEffiPdf (string n, vector<Variable*> obses, vector<Variable*> coeffs, vector<Variable*> offsets, unsigned int maxDegree) 
  : GooPdf(0, n) 
{
  ...
  GET_FUNCTION_ADDR(ptr_to_MultiPolyEffi);
  initialise(pindices); 
}

  __host__ fptype PolyEffiPdf::integrate (fptype lo, fptype hi) const {
  unsigned int* indices = host_indices+parameters; 
  fptype lowestDegree = indices[1]; 
  fptype ret = 0; 
  for (int i = 2; i < indices[0] ; ++i) {
    fptype powerPlusOne = lowestDegree + i - 2; 
    fptype curr = (POW(hi, powerPlusOne)-POW(lo, powerPlusOne))/powerPlusOne; 
 ...
 */
__host__ fptype PolyEffiPdf::getCoefficient (int coef) const {
  // NB! This function only works for single PolyEffis. 
  if (1 != observables.size()) {
    std::cout << "Warning: getCoefficient method of PolyEffiPdf not implemented for multi-dimensional PolyEffis. Returning zero, which is very likely wrong.\n"; 
    return 0; 
  }
