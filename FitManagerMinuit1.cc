...
void FitManager::runMinos () { 
  ...
  double plist[1];
 if (0 < overrideCallLimit) {
    std::cout << "Calling MINOS with call limit " << overrideCallLimit << std::endl; 
    plist[0] = overrideCallLimit;
    minuit->mnexcm("MINOS", plist, 1, err);
  }else{
   plist[0] = 0;
   minuit->mnexcm("MINOS", 0, 1, err);
  }
}

void FitManager::runImprove () { 
  assert(minuit);
  host_callnumber = 0;
  int err = 0; 
  double plist[1];
 if (0 < overrideCallLimit) {
    std::cout << "Calling IMPROVE with call limit " << overrideCallLimit << std::endl; 
    plist[0] = overrideCallLimit;
    minuit->mnexcm("IMPROVE", plist, 1, err);
  }else{
   plist[0] = 0;
   minuit->mnexcm("IMPROVE", 0, 1, err);
  }
}

void FitManager::runCommand ( const char* command) { 
  assert(minuit);
  host_callnumber = 0;
  int err = 0; 
  double plist[1];
 if (0 < overrideCallLimit) {
    std::cout << "Calling "<< command <<" with call limit " << overrideCallLimit << std::endl; 
    plist[0] = overrideCallLimit;
    minuit->mnexcm(command, plist, 1, err);
  }else{
   plist[0] = 0;
   minuit->mnexcm(command, 0, 1, err);
  }
}
