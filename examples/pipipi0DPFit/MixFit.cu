...
GooPdf* makeEfficiencyPoly () {
  ...
  offsets.push_back(constantOne); offsets.push_back(constantOne); 
...
//  dtop0pp->_tau = new Variable("tau", 0.4116, 0.001, 0.300, 0.500);
  ...
  if (!drop_f0_980)  dtop0pp->resonances.push_back(f0_980); 
  if (!drop_f0_1370) dtop0pp->resonances.push_back(f0_1370); 
  if (!drop_f0_1500) dtop0pp->resonances.push_back(f0_1500); 
  if (!drop_f0_1710) dtop0pp->resonances.push_back(f0_1710); 
  if (!drop_f2_1270) dtop0pp->resonances.push_back(f2_1270); 
  if (!drop_f0_600)  dtop0pp->resonances.push_back(f0_600); 
...
//      resolution = new ThreeGaussResolution(coreFrac, constantOne, constantZero, coreScaleFactor, constantZero, tailScaleFactor, constantZero, constantOne); 
  ...
  TddpPdf* mixPdf = 0; 
  if (massd0) mixPdf = new TddpPdf("mixPdf", dtime, sigma, m12, m13, eventNumber, dtop0pp, resList, eff, massd0, wBkg1);
  else mixPdf = new TddpPdf("mixPdf", dtime, sigma, m12, m13, eventNumber, dtop0pp, resolution, eff, wBkg1);
...
void loadDataFile (char* fname, UnbinnedDataSet** setToFill, int effSkip) {
  ...
  while (!reader.eof()) {
    ...
    if (deltam) {
      ...
      if (delm >= 0.1454 + 0.0003*deltam_upper_window) continue;
      if (delm <= 0.1454 + 0.0003*deltam_lower_window) continue;
    }
    ...
    if (loM23Sigma) {
      ...
      if (currM23 < 1.5) loM23Sigma->Fill(sigma->value);
      else hiM23Sigma->Fill(sigma->value);
      //std::cout << "Filled sigma with " << sigma->value << " " << currM23 << std::endl; 
    }
    if (wSig0->value > 1.0) std::cout << "Problem with event " << (*setToFill)->getNumEvents() << ", too-large signal weight " << wSig0->value << std::endl; 
...
GooPdf* makeSignalJSU_gg (int idx, bool fixem = true) {
  ...
  Variable* js_meana = new Variable(strbuffer, 0.0593, 0.01, 0, 0.2); js_meana->fixed = fixem;
  sprintf(strbuffer, "js_sigma_%i", jsugg_num);
  Variable* js_sigma = new Variable(strbuffer, 0.000474, 0.0001, 0, 0.001);  js_sigma->fixed = fixem;
  sprintf(strbuffer, "js_gamma_%i", jsugg_num);
  Variable* js_gamma = new Variable(strbuffer, -10.1942, 1, -30, 0);  js_gamma->fixed = fixem;
  sprintf(strbuffer, "js_delta_%i", jsugg_num);
  Variable* js_delta = new Variable(strbuffer, 1.4907, 0.1, 0.5, 5);    js_delta->fixed = fixem;
  sprintf(strbuffer, "frac_jsu_%i", jsugg_num);
  Variable* frac_jsu = new Variable(strbuffer, 0.9516, 0.01, 0.5, 1.0);    frac_jsu->fixed = fixem;
  sprintf(strbuffer, "frac_ga1_%i", jsugg_num);
  Variable* frac_ga1 = new Variable(strbuffer, 0.001845, 0.0005, 0.0001, 0.3);  frac_ga1->fixed = fixem;
  sprintf(strbuffer, "g1_meana_%i", jsugg_num);
  Variable* g1_meana = new Variable(strbuffer, 0.2578, 0.003, 0.1, 0.5);    g1_meana->fixed = fixem;
  sprintf(strbuffer, "g1_sigma_%i", jsugg_num);
  Variable* g1_sigma = new Variable(strbuffer, 0.03086, 0.01, 0.005, 0.25);   g1_sigma->fixed = fixem;
  sprintf(strbuffer, "g2_meana_%i", jsugg_num);
  Variable* g2_meana = new Variable(strbuffer, 0.32, 0.01, 0.1, 0.5);      g2_meana->fixed = fixem;
  sprintf(strbuffer, "g2_sigma_%i", jsugg_num);
  Variable* g2_sigma = new Variable(strbuffer, 0.05825, 0.01, 0.01, 0.1);   g2_sigma->fixed = fixem;
...
GooPdf* makeMikhailJSU_gg (bool fixem = true) {
  ...
  Variable* js_meana = new Variable("js_meana", 0.0593279, 0.01, 0, 0.2); js_meana->fixed = fixem;
  Variable* js_sigma = new Variable("js_sigma", 0.000474171, 0.0001, 0, 0.001);  js_sigma->fixed = fixem;
  Variable* js_gamma = new Variable("js_gamma", -10.1942, 1, -30, 0);  js_gamma->fixed = fixem;
  Variable* js_delta = new Variable("js_delta", 1.4907, 0.1, 0.5, 5);    js_delta->fixed = fixem;
  Variable* frac_jsu = new Variable("frac_jsu", 0.951638, 0.01, 0.5, 1.0);    frac_jsu->fixed = fixem;
  Variable* frac_ga1 = new Variable("frac_ga1", 0.0184522, 0.00001, 0.001, 0.3);  frac_ga1->fixed = fixem;
  Variable* g1_meana = new Variable("g1_meana", 0.257796, 0.003, 0.1, 0.5);    g1_meana->fixed = fixem;
  Variable* g1_sigma = new Variable("g1_sigma", 0.0308619, 0.01, 0.005, 0.25);   g1_sigma->fixed = fixem;
  Variable* g2_meana = new Variable("g2_meana", 0.319952, 0.01, 0.1, 0.5);      g2_meana->fixed = fixem;
  Variable* g2_sigma = new Variable("g2_sigma", 0.0582524, 0.01, 0.01, 0.1);   g2_sigma->fixed = fixem;
...
GooPdf* makeSigmaMap () {
  ...
  for (int i = 0; i < numSigmaBins; ++i) {
    ...
    */ 
    if (0 == sigma_data[i]->numEvents()) js->setParameterConstantness(true); 
    else {
...
void makeDalitzPlots (GooPdf* overallSignal, string plotdir = "./plots_from_mixfit/") {
  ...
  TH2F dalitzpm_dat_hist("dalitzpm_dat_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  TH2F dalitzpm_pdf_hist("dalitzpm_pdf_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  dalitzpm_pdf_hist.SetStats(false); 

  TH2F dalitzp0_dat_hist("dalitzp0_dat_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  TH2F dalitzp0_pdf_hist("dalitzp0_pdf_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  dalitzp0_pdf_hist.SetStats(false); 

  TH2F dalitzm0_dat_hist("dalitzm0_dat_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  TH2F dalitzm0_pdf_hist("dalitzm0_pdf_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  for (int i = 1; i <= m12->numbins; ++i) {
    for (int j = 1; j <= m13->numbins; ++j) {
      ...
      if (curr < maxBinContent) continue;
      maxBinContent = curr;
      bestI = i;
      bestJ = j; 
    }
  ...
  for (int half = 0; half < division; ++half) {
    ...
    for (int k = 0; k < sigma->numbins; ++k) {
      ...
      if (0 == k) {
	if (signalDalitz) signalDalitz->setDataSize(currData.getNumEvents(), 6);
	if (incsum1) incsum1->setDataSize(currData.getNumEvents(), 6);
	if (incsum2) incsum2->setDataSize(currData.getNumEvents(), 6);
	if (incsum3) incsum3->setDataSize(currData.getNumEvents(), 6);
	if (incsum4) incsum4->setDataSize(currData.getNumEvents(), 6);
	if (incsum5) incsum5->setDataSize(currData.getNumEvents(), 6);
	if (incsum6) incsum6->setDataSize(currData.getNumEvents(), 6);
      }
  ...
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzpm_dat_log.png").c_str()); foodal->SetLogz(false);
  dalitzp0_dat_hist.Draw("colz"); 
  foodal->SaveAs((plotdir + "/dalitzp0_dat.png").c_str());
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzp0_dat_log.png").c_str()); foodal->SetLogz(false);
  dalitzm0_dat_hist.Draw("colz"); 
  foodal->SaveAs((plotdir + "/dalitzm0_dat.png").c_str());
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzm0_dat_log.png").c_str()); foodal->SetLogz(false);

  dalitzpm_pdf_hist.Draw("colz"); 
  foodal->SaveAs((plotdir + "/dalitzpm_pdf.png").c_str()); 
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzpm_pdf_log.png").c_str()); foodal->SetLogz(false);
  ...
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzpm_pdf_matched_log.png").c_str()); foodal->SetLogz(false);
  ...
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzp0_pdf_log.png").c_str()); foodal->SetLogz(false);
  ...
  foodal->SetLogz(true); foodal->SaveAs((plotdir + "/dalitzm0_pdf_log.png").c_str()); foodal->SetLogz(false);
  ...
  for (int i = 1; i <= m12->numbins; ++i) {
    ...
    for (int j = 1; j <= m13->numbins; ++j) {
      ...
      if (!cpuDalitz(m12loedge, m13loedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitzpm_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m13loedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitzpm_dat_hist.SetBinContent(i, j, 0); continue;}
      double m13hiedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j); 
      if (!cpuDalitz(m12loedge, m13hiedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitzpm_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m13hiedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitzpm_dat_hist.SetBinContent(i, j, 0); continue;}
    ...
    for (int j = 1; j <= m13->numbins; ++j) {
      ...
      if (!cpuDalitz(m12loedge, m23loedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzp0_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m23loedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzp0_dat_hist.SetBinContent(i, j, 0); continue;}
      double m23hiedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j); 
      if (!cpuDalitz(m12loedge, m23hiedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzp0_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m23hiedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzp0_dat_hist.SetBinContent(i, j, 0); continue;}
    ...
    for (int j = 1; j <= m13->numbins; ++j) {
      double m23loedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j-1); 
      if (!cpuDalitz(m12loedge, m23loedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzm0_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m23loedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzm0_dat_hist.SetBinContent(i, j, 0); continue;}
      double m23hiedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j); 
      if (!cpuDalitz(m12loedge, m23hiedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzm0_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m23hiedge, _mD0, piPlusMass, piZeroMass, piPlusMass)) {dalitzm0_dat_hist.SetBinContent(i, j, 0); continue;}
...
GooPdf* makeSigmaHists () {
  ...
  while (!reader.eof()) {
    reader >> dummy;
    reader >> m23; 
    reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; 
    reader >> dtime->value;
    reader >> sigma->value; 

    reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; 
    reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; reader >> dummy; 
...
GooPdf* makeOverallSignal () {
  ...
  if (useHistogramSigma) sig0_jsugg = makeSigmaHists();
  else sig0_jsugg = makeBkg_sigma_strips(0); 
  sig0_jsugg->addSpecialMask(PdfBase::ForceSeparateNorm); 
...
void runGeneratedMCFit (char* fname, int genResolutions, double dplotres) { 
  ...
  if (0 != genResolutions) {
	...
	if (100 > smearedData->numEvents()) std::cout << "Before smear: " << m12->value << " " << m13->value;
	...
	m13->value += smear2; 
	//}
      if (100 > smearedData->numEvents()) std::cout << " After smear: " << m12->value << " " << m13->value << "\n"; 
...
GooPdf* makeBkg2_sigma () {
  ...
  CrystalBallPdf* bkg2_sigma_js = new CrystalBallPdf("bkg2_sigma_js", sigma, bkg2_sigma_js_meana, bkg2_sigma_js_sigma, bkg2_sigma_js_gamma, bkg2_sigma_js_delta);
...
GooPdf* makeBkg4_sigma () {
  ...
  CrystalBallPdf* bkg4_sigma_js = new CrystalBallPdf("bkg4_sigma_js", sigma, bkg4_sigma_js_meana, bkg4_sigma_js_sigma, bkg4_sigma_js_gamma, bkg4_sigma_js_delta);
...
GooPdf* makeBkg3_sigma () {
  ...
  CrystalBallPdf* bkg3_sigma_js = new CrystalBallPdf("bkg3_sigma_js", sigma, bkg3_sigma_js_meana, bkg3_sigma_js_sigma, bkg3_sigma_js_gamma, bkg3_sigma_js_delta);
  //JohnsonSUPdf* bkg3_sigma_js = new JohnsonSUPdf("bkg3_sigma_js", sigma, bkg3_sigma_js_meana, bkg3_sigma_js_sigma, bkg3_sigma_js_gamma, bkg3_sigma_js_delta);
  ...
  JohnsonSUPdf* bkg3_sigma_g2 = new JohnsonSUPdf("bkg3_sigma_g2", sigma, bkg3_sigma_g2_meana, bkg3_sigma_g2_sigma, bkg3_sigma_g2_gamma, bkg3_sigma_g2_delta);
...
GooPdf* makeGaussianTimePdf (int bkg) {
  ...
  if (3 == bkg) weights.push_back(frac_ga3);
  comps.clear(); 
  comps.push_back(g2);
  if (3 == bkg) comps.push_back(g3);
  comps.push_back(g1);
...
GooPdf* makeBkg2DalitzPdf (bool fixem = true) {
  ...
  if (Parameter == bkg2Model) {
    ...
    TrigThresholdPdf* bkg2_loZ = new TrigThresholdPdf("bkg2_loZ", m12, m13, minDalitzZ, bkg2_decZmin, bkg2_conZmin, massSum, false);
...
GooPdf* makeBkg3Eff () {
  ...
  while (!reader.eof()) {
    ...
    for (int i = 0; i < 16; ++i) reader >> dummy;
...
GooPdf* makeBackground4DalitzParam () {
  ...
  TrigThresholdPdf* bkg4_loZ = new TrigThresholdPdf("bkg4_loZ", m12, m13, minDalitzZ, bkg4_decZmin, bkg4_conZmin, massSum, false);
...
GooPdf* makeBkg4DalitzPdf (bool fixem = true) {
  ...
  if (useBackground4Hist) bkg4_dalitz = makeBackgroundHistogram(4);
...
void runEfficiencyFit (int which) {
  ...
  TH2F dalitz_dat_hist("dalitz_dat_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  TH2F dalitz_pdf_hist("dalitz_pdf_hist", "", m12->numbins, m12->lowerlimit, m12->upperlimit, m13->numbins, m13->lowerlimit, m13->upperlimit);
  ...
  */
  for (int i = 1; i <= m12->numbins; ++i) {
    for (int j = 1; j <= m13->numbins; ++j) {
      ...
      if (currNormEff >= 50) std::cout << "High efficiency: " << currNormEff << " " << i << " " << j << std::endl; 
    }
  }
  ...
  for (int i = 1; i <= m12->numbins; ++i) {
    ...
    for (int j = 1; j <= m13->numbins; ++j) {
      double m13loedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j-1); 
      if (!cpuDalitz(m12loedge, m13loedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitz_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m13loedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitz_dat_hist.SetBinContent(i, j, 0); continue;}
      double m13hiedge = m13->lowerlimit + ((m13->upperlimit - m13->lowerlimit) / m13->numbins) * (j); 
      if (!cpuDalitz(m12loedge, m13hiedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitz_dat_hist.SetBinContent(i, j, 0); continue;}
      if (!cpuDalitz(m12hiedge, m13hiedge, _mD0, piZeroMass, piPlusMass, piPlusMass)) {dalitz_dat_hist.SetBinContent(i, j, 0); continue;}
      ...
      if ((currm13 > 2) || (currm12 > 2)) hiM23pullplot.Fill(pull); 
      else loM23pullplot.Fill(pull); 
    }
  }
...
void makeTimePlots (char* fname) {
  ...
  for (int i = 0; i < 6; ++i) {
    TLine* currLine = new TLine(massd0->lowerlimit + (i+0)*(massd0->upperlimit - massd0->lowerlimit)/6, dtime->lowerlimit+0.09,
...
bool parseArg (string arg) {
 ...
 start:
  size_t pos = arg.find("="); 
  if (pos == string::npos) goto badArg;

  string variable = arg.substr(0, pos);
  string assigned = arg.substr(pos+1); 
  double number   = atof(assigned.c_str());

  if ((0 == variable.size()) || (0 == assigned.size())) goto badArg; 

  if (variable == "luckyFrac") luckyFrac = number; 
  else if (variable == "mesonRad") mesonRad = number; 
  else if (variable == "normBins") normBinning = (int) floor(number + 0.5); 
  ...
  else if (variable == "lowerTime") lowerTime = number;
    ...
    else if (assigned == "f0_980")   drop_f0_980   = true;
    else if (assigned == "f0_1370")  drop_f0_1370  = true;
    else if (assigned == "f0_1500")  drop_f0_1500  = true;
    else if (assigned == "f0_1710")  drop_f0_1710  = true;
    else if (assigned == "f2_1270")  drop_f2_1270  = true;
    else if (assigned == "f0_600")   drop_f0_600   = true;
    else goto badArg; 
  }
  else if (variable == "histSigma") useHistogramSigma = (assigned == "yes"); 
  else if (variable == "makePlots") makePlots = (assigned == "yes"); 
  else if (variable == "bkg2Model")  {
    if      (assigned == "histogram") bkg2Model = Histogram;
    else if (assigned == "parameter") bkg2Model = Parameter;
    else if (assigned == "sideband")  bkg2Model = Sideband;
    else goto badArg; 
  }
  else if (variable == "bkg3Hist")  useBackground3Hist = (assigned == "yes"); 
  else if (variable == "bkg4Hist")  useBackground4Hist = (assigned == "yes"); 
  else if (variable == "bkgHistBins") bkgHistBins = (int) floor(number + 0.5); 
  else if (variable == "varyParameterUp") paramUp = assigned;
  else if (variable == "varyParameterDn") paramDn = assigned;
...
int main (int argc, char** argv) {
  ...
  case 4: 
    for (int i = 3; i < argc; ++i) assert(parseArg(argv[i])); 
  ...
  case 5: 
    ...
    break; 
  case 6: runBackgroundSigmaFit(atoi(argv[2])); break; 
  case 7: writeBackgroundHistograms(atoi(argv[2])); break; 
  case 8: 
    genResolutions = atoi(argv[3]);
    if (DplotRes & genResolutions) dplotres = atof(argv[4]);
